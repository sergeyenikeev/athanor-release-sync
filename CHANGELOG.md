...
- Уточнение к [1.9.1]: видео пересобирается итеративно (v22+, блокеры [1.10.0]
  в кадре метрик); актуальный артефакт — **~2:49** (< 3:00), точная длительность/
  размер — mvhd текущего `video/*.mp4`. Точные цифры в [1.9.1] — промежуточные.
