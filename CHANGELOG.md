...
## [1.9.1] — 2026-07-20 (v20: честность метрик + синхронизация клеймов по итогам 5 независимых оценок)

### Изменено
- **Честность метрик** (`tests/score.py`): `draft_acceptance` больше не константа
  `1.0` — принятие черновиков человеком в автопрогоне не замеряется (черновики
  создаются в `awaiting_approval`), теперь `null` = «не замерено, замер пилота»;
  `model` в агрегате = `null` для rule-движка (LLM не вызывалась, llm_calls=0).
  Зеркала канонического прогона `eval_20260719T_fixed` перегенерированы
  (`metrics.json`, `results_summary.md`, `evaluation_report.html`, csv);
  **P/R/F1, тайминги и per-scenario выходы не изменились** — изменены только
  `draft_acceptance` (1.0→null), `model` (→null), `target_met.draft_acceptance`
  (true→null). Итог по целям: «5 из 6 достигнуты» → **«4 из 6 достигнуты;
  принятие черновиков и полезность сводки — пилот»** (README, дек сл. 8/9/11).
- **Live LLM-прогон выведен в подачу**: `results/live_llm_report.md`
  (`eval_glm52_20260720`, z-ai/glm-5.2 через OpenRouter: сводка F1=94.1%,
  блокеры 100%, решения 84.6%, поручения 18.2% при строгом текстовом матчинге)
  теперь явно упомянут в README (раздел «Live LLM-прогон» + провенанс моделей)
  и в деке (сл. 8 заметки + сл. 16 отдельная строка) как честная нижняя
  граница; семантический матчинг — пилот.
- **README.md**: «Быстрый старт» (3 команды) сразу после шапки; длительность
  видео `2:48.28` → **2:30** и монтаж обновлён до фактического ([1.9.0]:
  F2=e2e_launch, F3=5 cloud-клипов, F6=e2e_hitl, F8=CLI); «Единственный файл в
  video/» → три файла (mp4, srt, subtitles); счётчик тестов в «Командах»
  117 → **126**; примечание «outbox/ создаётся при первом прогоне».
- **`scripts/gen_reports.py`**: `2:48.28` → `2:30`, в генератор добавлены
  боевой e2e-пруф и монтажный лист — `results/demo_scenario.md` теперь
  воспроизводится генератором байт-в-байт (правки мимо генератора устранены).
- **`.gitignore`**: whitelist пруфов боевых e2e-прогонов Ouroboros
  (`results/scratch/ouroboros_{hitl_e2e,evolution,edit_comment,mcp_hitl}/`,
  `OUROBOROS_RUNS_SUMMARY.md`) — README/docs/CHANGELOG ссылаются на них,
  теперь они попадают в публичный репозиторий.
- **Дек** (`build_results.py`, внешний репо): сл. 3 заголовок «Все функции
  реализованы» → «11 из 13 полностью, 2 — частично» (соответствует таблице);
  сл. 8 — «4/6 целей», черновики «11/11 сформировано; принятие — пилот» 📅,
  TP/FP/FN добавлены к метрикам, заметки переписаны (устранено
  самопротиворечие «Не скрываем не-100%: … 100%», добавлен live-LLM);
  сл. 9/11 — черновики и «4/6»; сл. 16 — строка live LLM-прогона.
  PPTX/PDF пересобраны (16 слайдов, 0 OOB), копии в `docs/` обновлены.

### Не изменено (правило честности)
- Видео каноническое **2:30.7** (= git HEAD, QR): невоспроизводимая локальная
  сборка 2:49.86 (расширенная озвучка отсутствует в make_video.py) в репозиторий
  не попала — архив `out/video/_archive_20260720_v249/` (внешний репо).
- Артефакты прогонов (`run.json`, `manifest.json`, выходы TB-сценариев,
  `results/scratch/ouroboros_*/result.json`) не переписывались.
