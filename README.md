...
> (`MCP_BACKEND=live`); расшифровки — файлы кейса. Боевой Outlook — тот же интерфейс, после ИБ-согласования.
...
| Боевые коннекторы Jira/Bitbucket/mail/Calendar | реализовано (`MCP_BACKEND=live`) | `mcp/_backends.py`, `test-instances/seed_*.py` |
| Боевой коннектор Outlook (MS Graph) | подготовлен интерфейс | `mcp/` (после пилота, `MCP_BACKEND=microsoft`) |
...
## Тесты и метрики
```bash
python scripts/run_tests.py                                   # 119 тестов, одна команда
...
- Боевой Outlook (Microsoft Graph) не подключён (нужна регистрация приложения в Azure
  + ИБ-согласование); Jira/Bitbucket/Confluence/mail/Calendar — реализованы и подключены
  через `MCP_BACKEND=live` (`mcp/_backends.py`, сидинг — `test-instances/seed_*.py`).
...
test-instances/ jira_server (REST v2), graph_server (MS Graph), bitbucket_server (Cloud REST 2.0), confluence_server (REST API v1), serve_all, discover_atlassian, seed_atlassian, seed_bitbucket, seed_confluence, gen_live_case
...
2. **Локальные тестовые инстансы** (`MCP_BACKEND=test`, `test-instances/`) — серверы с
   реальными контрактами Jira REST v2, Microsoft Graph, Bitbucket Cloud REST 2.0, Confluence
   Cloud REST API v1. Офлайн, детерминированно.
...
**mail/Calendar без Azure:** личный Outlook.com требует регистрации приложения в
Azure (OAuth2); mail работает через IMAP + пароль приложения (2FA → myaccount.google.com/apppasswords),
календарь — публичный iCal URL (`calendar.google.com → Settings → Integrate calendar`).
