...
Известное ограничение: `edit` правит метаполя (`to_role`, `due`, `subject`, `body`),
но не перерендеривает тело письма автоматически — метаданные корректны, `body` может
устареть. Боевой прогон `10ffd02e` (edit+comment+approve) — `results/scratch/ouroboros_edit_comment/`.
...
python scripts/run_evaluation.py --engine rule                # корзина + метрики + эволюция (rule-baseline)
python tests/run_basket.py --engine llm --run-id llm_real_v1  # real-LLM прогон (jetnight-pro, нужен ключ)
python tests/run_basket.py --engine llm --mock --run-id llm_mock_v1  # LLM-прогон через mock (без сети)
```

Два прогона корзины:
- **Rule-baseline** (`eval_20260719T_fixed`): 17/17 success, F1=100% — детерминированный
  движок, доказывает конвейер и инфраструктуру. Эталон: TB-01..12 — baseline-корректность,
  TB-13..17 — ручная разметка (`manual_expected=True`).
- **Real-LLM** (`llm_real_v1`, jetnight-pro): **6 success / 10 partial / 1 failed**,
  решения F1=64%, среднее 5.5 с — реальные метрики LLM-извлечения с ошибками
  (LLM нормализует время глаголов: «подготовлю»→«подготовить»).

Recall на реальных расшифровках и полезность сводки — замер пилота (независимый
оценщик). Боевые прогоны через Ouroboros — `results/scratch/ouroboros_*/` +
`OUROBOROS_RUNS_SUMMARY.md`.
