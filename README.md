...
> Публичный репозиторий: <https://github.com/sergeyenikeev/athanor-release-sync>
> (MIT, stdlib-only). Клон: `git clone https://github.com/sergeyenikeev/athanor-release-sync`.
...
**Быстрый старт (офлайн, без ключей и сети):**

```bash
git clone https://github.com/sergeyenikeev/athanor-release-sync
cd athanor-release-sync
python scripts/run_demo.py    # демо end-to-end: сводка → решения → поручения → HITL → память
python scripts/run_tests.py   # 126 тестов (unittest)
```
...
|---|---|---|
| Сводка перед релиз-синком (детерминированная) | реализовано | `src/athanor/summary.py`, `results/runs/eval_20260719T_fixed/TB-01/output.md` |
...
(или `→ rejected/failed`). CLI-команды (доступны через чат Ouroboros → навык
`release_sync`):
```bash
# outbox/ создаётся при первом прогоне (python scripts/run_demo.py или cli run)
python -m athanor.cli approve --draft outbox/<id>.json --execute   # подтвердить + исполнить
python -m athanor.cli reject  --draft outbox/<id>.json --reason "…" # отклонить
python -m athanor.cli edit    --draft outbox/<id>.json --to-role SRE --due 2026-07-10  # правка
python -m athanor.cli comment --draft outbox/<id>.json --text "…"   # комментарий
```
Без `approve` исполнение невозможно (статус → `failed`). Боевой e2e через Ouroboros
(HITL approve/reject в чате) — `results/scratch/ouroboros_hitl_e2e/`
(task `de797d3f`, 40 rounds, tier=solved).
...
## Тесты и метрики
```bash
python scripts/run_tests.py                                   # 126 тестов, одна команда
...
| Принятые черновики | ≥ 60% | не замерено: 11/11 сформировано (awaiting_approval), принятие — решение человека | 📅 пилот |
| Время подготовки TO BE | ≤ 3 мин | <1 с end-to-end (2 мс среднее / 3 мс p95 cycle) | ✅ |

4 из 6 целей измерены и достигнуты (rule-бейзлайн); полезность сводки и принятие
черновиков — замер пилота (независимый оценщик).
Подробнее — `docs/testing.md`, `docs/evaluation.md`. Результаты — `results/` (`metrics.json`,
`results_summary.md`).

### Live LLM-прогон (реальная модель, честный замер)

Помимо канонического rule-бейзлайна корзина прогнана на **реальной LLM**
(`z-ai/glm-5.2` через OpenRouter): отчёт — `results/live_llm_report.md`, прогон —
`results/runs/eval_glm52_20260720`. Итог: сводка F1 = 94.1%, блокеры F1 = 100%,
решения F1 = 84.6%, поручения F1 = 18.2% при строгом текстовом сопоставлении
4 полей (парафразы не засчитываются: «подготовить отчёт» ≠ «подготовлю отчёт»,
`source: расшифровка` ≠ `source: APP-412`). Это честная нижняя граница качества
LLM-извлечения; семантическое сопоставление (LLM-judge) и разметка реальных
расшифровок — план пилота. Канонические 100% rule-движка — регрессионный тест
конвейера на синтетической корзине, выровненной с детерминированным извлечением,
а не оценка качества LLM (см. примечание в `results_summary.md`).

**Какая модель где использовалась:** канонические метрики корзины — rule-движок
(LLM не вызывалась, `model: null` в `metrics.json`); live LLM-замер —
`z-ai/glm-5.2` (OpenRouter); прогоны агента Ouroboros — `dec66d75`
(Claude Opus 4.8) и `a5336602` (jetnight-opus); текущая конфигурация агента —
`jetnight-pro` (GLM 5.2, jet-night router, см. `.env.example`).

## Замер AS IS/TO BE на реальных данных

| Параметр | AS IS (вручную) | TO BE (с агентом) | Статус |
|---|---|---|---|
| Время подготовки к релиз-синку | 15 мин (замер) | <1 с end-to-end (rule-baseline, 2 мс среднее / 3 мс p95 cycle); реальный прогон Ouroboros — $0.4337, 16 rounds | AS IS и TO BE замерены ✅ |
...
| Принятые черновики (HITL) | — (не релевантно: метрика агента) | 11/11 сформировано (awaiting_approval); принятие человеком не замерено | 📅 пилот |
...
python test-instances/seed_more.py              # Jira (+7) + Bitbucket (+8 PR) + Confluence (+8) + Gmail (+6) + Calendar .ics (35 событий, рабочие дни)
```
...
## Демо-видео
`video/Athanor_Ouroboros_Project_Results_Demo.mp4` (2:30, 1920×1080,
H.264/AAC) — финальное демо-видео (< 3 мин, критерий «ДЕМО-видео» 30%).
Реальные скринкасты в монтаже (см. монтажный лист в `results/demo_scenario.md`):
F2 — запуск в UI Ouroboros (Playwright-запись боевого прогона `de797d3f`,
run + HITL, tier=solved); F3 — 5 cloud-клипов сбора контекста
(Calendar · mail · Jira · Bitbucket · Confluence Cloud); F6 — HITL
approve/reject в чате Ouroboros (`e2e_hitl`); F8 — эволюция навыка v1→v2→rollback
(CLI). Прогоны агента: `dec66d75` (Claude Opus 4.8, $0.43, 16 rounds) —
`results/scratch/ouroboros_demo/`; `a5336602` (jetnight-opus, 8 rounds, $0) —
`results/scratch/ouroboros_demo_e2e_jetnight/` (result.json, F2_real_clip.mp4,
cloud_*.mp4). В `video/` — три файла: MP4, субтитры `.srt` и версия с вшитыми
субтитрами `*_subtitles.mp4`. QR-код презентации ведёт на
`/blob/main/video/Athanor_Ouroboros_Project_Results_Demo.mp4` (raw URL HTTP 200
после перевода репозитория в Public).
Сборщик (PIL-рендер кадров + edge-tts DmitryNeural и/или готовый MiniMax MP3 через `VOICEOVER_MP3` + сведение ffmpeg) — во внешнем
...
python scripts/run_tests.py                # все тесты (unittest, 126)
