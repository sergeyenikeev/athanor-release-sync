...
|---|---|---|
| Сводка перед релиз-синком (детерминированная) | реализовано | `src/athanor/summary.py`, `results/runs/eval_20260720T_blockers/TB-01/output.md` |
...
| Блокеры (структурированные: письма, конфликты, решения синка) | реализовано | `src/athanor/enrich.py` (Blocker), TB-02/03/04/12 |
...
| Замер AS IS/TO BE на реальных данных | реализовано (AS IS: время 15 мин, полезность ручной сводки 20%; TO BE: корзина+Ouroboros, полезность сводки агента 5/5 — замер пилота; recall на реальных расшифровках — пилот) | раздел «Замер AS IS/TO BE», `results/metrics.json`, `results/scratch/ouroboros_demo/` |
...
## Тесты и метрики
```bash
python scripts/run_tests.py                                   # 131 тест, одна команда
python scripts/run_evaluation.py --engine rule                # корзина + метрики + эволюция
```

### Цели Project Proposal vs факт MVP (rule-baseline + live LLM)
| Цель | План | Rule-baseline | Live LLM (GLM 5.2, семантич.) | Статус |
|---|---|---|---|---|
| Поручения precision | ≥ 85% | 100% | 90.9% | ✅ |
| Поручения recall | ≥ 80% | 100% | 90.9% | ✅ |
| Поручения F1 | ≥ 82% | 100% | 90.9% | ✅ |
| Полезность сводки | ≥ 4/5 | 5/5 — замер пилота, независимый оценщик (AS IS ручная база: 20%) | — | ✅ |
| Принятые черновики | ≥ 60% | 100% (11/11) | — | ✅ |
| Время подготовки TO BE | ≤ 3 мин | <1 с end-to-end (2 мс среднее / 3 мс p95 cycle) | 10.9 с среднее | ✅ |

Все 6 целей измерены и достигнуты; полезность сводки агента — 5/5 (замер пилота, независимый оценщик), AS IS-база ручной сводки — 20% (1/5).
Live-метрики GLM 5.2 подтверждены независимым семантическим LLM-судьёй (gpt-4o-mini, T=0,
два прохода — вердикты идентичны; строгое текстовое сопоставление даёт 18.2% из-за парафраз) —
методика и вердикты: `results/live_llm_report.md`, `results/runs/eval_glm52_20260720/semantic_judge/`.
...
| Полезность сводки (≥4/5) | 20% (1/5, замер ручной сводки) | 5/5 (замер пилота, независимый оценщик) | AS IS и TO BE замерены ✅ |
...
`results/scratch/ouroboros_demo/`. Поручения/черновики — метрики
качества агента, для ручного процесса (AS IS) не релевантны. Полезность ручной
сводки (AS IS) — 20% (1/5); полезность сводки агента — 5/5 (замер пилота,
независимый оценщик). Recall на реальных расшифровках — план пилота.
...
## Демо-видео
`video/Athanor_Ouroboros_Project_Results_Demo.mp4` (2:53.01, 1920×1080,
...
python scripts/run_tests.py                # все тесты (unittest, 131)
...
python tests/score.py --run results/runs/eval_20260720T_blockers --mirror   # метрики каноничного прогона + артефакты
