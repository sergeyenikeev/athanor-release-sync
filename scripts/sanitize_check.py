...
memory/, examples/, skills/ на ПДн и секреты.
...
TARGETS = [_ROOT / "test-basket", _ROOT / "memory", _ROOT / "examples", _ROOT / "skills"]
EXCLUDE = {".git", "__pycache__", "results", "node_modules", "npm-cache", ".venv"}
...
SYNTH_OK = ["Тимлид", "SRE", "Владелец продукта", "Разработчик A", "Разработчик B", "Атакующий",
...
def _scan_file(path: Path) -> list[dict]:
    ...
    for name, rx in DETECTORS:
        for m in rx.finditer(text):
            ...
            # фильтр ложных срабатываний: «Атакующий» — роль, не ПДн; APP-/OPS- — синтетика
...
def main() -> int:
    ...
    lines = [
        ...
        "",
        f"- Сканировано файлов: **{scanned}** в test-basket/, memory/, examples/, skills/",
