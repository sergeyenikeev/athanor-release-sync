...
SCENARIOS: list[dict] = [
    # TB-01 — обычный релиз-синк, полные консистентные данные
    {
        ...
        "tracker": {"issues": [
            {"key": "APP-412", "title": "Миграция схемы оплат", "status": "в работе", "assignee_role": "Разработчик backend"},
    ...
    # TB-02 — один блокер из письма
    {
        ...
        "mail": {"messages": [{"id": "M1", "from_role": "Разработчик frontend", "date": "2026-07-02",
            "subject": "Смежный сервис не задеплоен", "body": "Блокер: смежный сервис оплат не задеплоен, релиз под риском."}]},
        "transcript": "Тимлид: Решение: ждём деплой смежного сервиса, потому что без него релиз невозможен\n",
        "seed": None,
    },
    # TB-03 — конфликт Jira «готово» ↔ письмо «блокер»
    {
        ...
        "tracker": {"issues": [
            {"key": "APP-412", "title": "Миграция схемы оплат", "status": "готово", "assignee_role": "Разработчик backend"},
        ], "prs": [{"number": 128, "title": "Схема оплат", "status": "слито", "review_days": 0, "issue_key": "APP-412"}]},
        "mail": {"messages": [{"id": "M1", "from_role": "SRE", "date": "2026-07-02",
    ...
    # TB-04 — расшифровка: 2 решения, 3 поручения, 2 идеи-дистрактора
    {
        ...
        "transcript": (
            ...
            "SRE: SRE, подтверди деплой смежного сервиса до 03.07\n"
    ...
    # TB-06 — поручение без явного владельца + перенос обязательства из памяти
    {
        ...
        "tracker": {"issues": [
            {"key": "OPS-77", "title": "Деплой смежного сервиса", "status": "открыто", "assignee_role": "SRE"},
        ], "prs": []},
        "mail": {"messages": []},
        "transcript": "Тимлид: надо бы задеплоить смежный сервис до релиза\n",
        "seed": SEED_ALFA,
    },
    ...
    # TB-08 — отменённое обязательство
    {
        ...
        "mail": {"messages": []},
        "transcript": "Тимлид: Решение: OPS-77 больше не нужен, потому что смежный сервис уже задеплоен\n",
    ...
    # TB-09 — несколько похожих задач (нет путаницы APP-412 vs APP-421)
    {
        ...
        "tracker": {"issues": [
            {"key": "APP-412", "title": "Миграция схемы оплат", "status": "в работе", "assignee_role": "Разработчик backend"},
            {"key": "APP-421", "title": "Миграция схемы возвратов", "status": "готово", "assignee_role": "Разработчик backend"},
        ], "prs": [{"number": 129, "title": "Схема возвратов", "status": "на ревью", "review_days": 1, "issue_key": "APP-421"}]},
        "mail": {"messages": []},
    ...
    # TB-12 — повторный прогон после обратной связи (формат v2)
    {
        ...
        "mail": {"messages": [{"id": "M1", "from_role": "Разработчик frontend", "date": "2026-07-02",
            "subject": "Смежный сервис не задеплоен", "body": "Блокер: смежный сервис оплат не задеплоен."}]},
        "transcript": "Тимлид: Решение: окно релиза 03.07 18:00–20:00, потому что стандартное окно\nРазработчик backend: я подготовлю release-notes по APP-412 до 03.07\n",
        "seed": SEED_ALFA,
    },
]
