...
JIRA_MORE = [
    {"proj": "APP", "basket_key": "APP-410", "summary": "Кэш платежей",
     "status": "готово", "role": "Разработчик backend"},
    {"proj": "APP", "basket_key": "APP-413", "summary": "Вебхуки оплат",
     "status": "в работе", "role": "Разработчик backend"},
    {"proj": "APP", "basket_key": "APP-422", "summary": "Реестр возвратов",
    ...
    {"proj": "OPS", "basket_key": "OPS-79", "summary": "Мониторинг payment-adapter",
...
def seed_bitbucket_more(jira_map):
    ...
    prs_spec = [
        {"title": "Кэш платежей", "branch": "feature/payment-cache",
         "issue": "APP-410", "file": "cache.txt", "content": b"payment-cache v1 (demo)\n",
         "summary": "Кэш платежей: in-memory cache для payment-service (демо Ouroboros)"},
        {"title": "Вебхуки оплат", "branch": "feature/payment-webhooks",
         "issue": "APP-413", "file": "webhooks.txt", "content": b"payment-webhooks v1 (demo)\n",
         "summary": "Вебхуки оплат: обработка callback от партнёра (демо Ouroboros)"},
        {"title": "Реестр возвратов", "branch": "feature/refund-registry",
         "issue": "APP-422", "file": "registry.txt", "content": b"refund-registry v1 (demo)\n",
         "summary": "Реестр возвратов: реестр + сверка (демо Ouroboros)"},
        ...
        {"title": "Мониторинг payment-adapter", "branch": "feature/payment-monitoring",
         "issue": "OPS-79", "file": "monitoring.txt", "content": b"payment-monitoring v1 (demo)\n",
         "summary": "Мониторинг payment-adapter: health-checks + алерты (демо Ouroboros)"},
        ...
        {"title": "Архитектура платежей v2", "branch": "docs/payment-arch-v2",
         "issue": "APP-412", "file": "arch-v2.md",
         "content": "# Архитектура платежей v2 (demo)\n".encode("utf-8"),
         "summary": "Архитектура платежей v2: RFC + диаграммы (демо Ouroboros)"},
    ]
    out = []
...
CONF_PAGES = [
    {"title": "RFC · Платежи v2",
     "storage": "<h1>RFC · Платежи v2</h1><p>Миграция схемы оплат на v2: разбиение на payment-core + payment-adapter.</p><p>Owner: Разработчик backend. Срок: 03.07.</p>"},
    {"title": "Postmortem · Инцидент 2026-06-30",
     "storage": "<h1>Postmortem · Инцидент 2026-06-30</h1><p>Симптом: деградация payment-adapter 30.06 14:00–15:30.</p><p>Причина: исчерпание connection pool. Action items: OPS-79, APP-413.</p>"},
    ...
    {"title": "Changelog · Альфа 2026.07",
     "storage": "<h1>Changelog · Альфа 2026.07</h1><ul><li>APP-412 миграция схемы оплат</li><li>APP-521 интеграция с партнёром</li><li>APP-421 схема возвратов</li></ul>"},
    {"title": "On-call · Альфа",
    ...
    {"title": "Retrospective · Спринт 12",
     "storage": "<h1>Retrospective · Спринт 12</h1><p>Что хорошо: релиз 03.07 в срок. Что улучшить: связь с SRE по payment-adapter раньше.</p>"},
    {"title": "Architecture · Платежи",
     "storage": "<h1>Architecture · Платежи</h1><p>Компоненты: payment-core, payment-adapter, partner-api. Зависимости: partner-api (внешний).</p>"},
    {"title": "Test Plan · Альфа 2026.07",
     "storage": "<h1>Test Plan · Альфа 2026.07</h1><p>Smoke: payment-core e2e. Регресс: возвраты, вебхуки. Нагрузка: 100 RPS.</p>"},
]
...
MAIL_MORE = [
    {"basket_id": "more-1", "role": "Разработчик backend", "subject": "Release-notes APP-412 готовы",
     "body": "Release-notes по APP-412 (Миграция схемы оплат) готовы, залил в Confluence. (Синтетика Ouroboros, from backend.)"},
    ...
    {"basket_id": "more-4", "role": "SRE", "subject": "Runbook релизов обновлён",
     "body": "Runbook релизов Альфа обновлён, добавлен шаг smoke-теста payment-core. (Синтетика Ouroboros, from SRE.)"},
    {"basket_id": "more-5", "role": "Разработчик backend", "subject": "Запрос на ревью PR Кэш платежей",
     "body": "PR «Кэш платежей» готов к ревью, нужен аппрув до 03.07. (Синтетика Ouroboros, from backend.)"},
...
CAL_EVENTS = [
    ...
    {"uid": "alpha-incident-3006@athanor", "summary": "Разбор инцидента · Альфа",
     ...
     "desc": "Разбор инцидента 30.06: деградация payment-adapter 14:00–15:30."},
