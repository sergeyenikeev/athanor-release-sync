...
> (`results/runs/eval_20260719T_fixed`).
...
## Дополнение до 17+ (`seed_extra_jira.py`)

Доводит число Jira-задач до 17+ (дополнительные задачи в проектах APP/OPS):

```bash
python test-instances/seed_extra_jira.py   # идемпотентно: пропускает существующие
```

| Система | Что добавляет | Всего в облаке |
|---|---|---|
| **Jira** | +3 задачи (APP-8 «Автотесты ППРБ» [QA], APP-9 «Панель мониторинга ППРБ» [frontend], OPS-7 «Проверка ППРБ-адаптера в staging» [SRE]) | **17** (KAN-1/2 + APP-1..9 + OPS-1..7) |
...
> расширенных данных — `results/runs/live_real_20260720T123248/` (10 PR, 10 Confluence,
> 14 писем — актуальный live-прогон; более ранние прогоны в `live_real_20260718T*`).
...
3d. **Библиотека сущностей тестовой корзины** в реальных облаках (`seed_basket.py` + `seed_extra_jira.py`): **17 Jira-задач**
    (KAN-1/2 + APP-1..9 + OPS-1..7), 10 PR Bitbucket, 10 Confluence-страниц, 14 писем Gmail — все 17 сценариев представлены своими
